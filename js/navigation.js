(function () {
  ...
  function shell(page, content) {
    ...
    return `<a class="skip-link" href="#main-content">Pular para o conteúdo</a>
      <div class="app-shell"><aside class="sidebar" aria-label="Menu principal"><a class="brand" href="index.html"><span class="brand-mark">A</span><span><strong>Aprova360</strong><small>Câmara de Itanhaém</small></span></a><nav class="sidebar-nav">${links}</nav><a class="sidebar-upgrade" href="planos.html"><span class="badge ${subscription.plan==='free'?'warning':'success'}">${planLabel}</span><b>${subscription.plan==='free'?'Desbloqueie seu potencial':'Experiência completa'}</b><small>${subscription.plan==='free'?'Conheça o Aprova360 Pro →':'Todos os recursos liberados'}</small></a><div class="sidebar-progress"><div><span>Progresso geral</span><b>${percent}%</b></div><div class="progress"><i style="width:${percent}%"></i></div></div></aside>
      <div class="mobile-overlay" data-action="close-menu"></div><div class="app-main"><header class="topbar"><button class="icon-button menu-toggle" data-action="toggle-menu" aria-label="Abrir menu">☰</button><div class="search"><span>⌕</span><label class="sr-only" for="global-search">Buscar</label><input id="global-search" placeholder="Buscar capítulos, questões e anotações…"></div><div class="top-actions"><button class="button button-secondary button-sm install-button" data-action="install-app" hidden>Instalar app</button><button class="icon-button" data-action="open-pomodoro" aria-label="Abrir cronômetro" title="Cronômetro">◷</button><button class="icon-button" data-action="toggle-theme" aria-label="Alternar tema" title="Alternar tema">◐</button><a class="profile-button" href="configuracoes.html"><span class="avatar">${escapeText(user.name).slice(0,2).toUpperCase()}</span><span>${escapeText(user.name)}<small>${planLabel}</small></span></a></div></header><main class="content" id="main-content">${content}</main></div></div>
